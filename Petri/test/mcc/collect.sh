...
#
#   bash Petri/test/mcc/collect.sh <date> EXAM... [--pages]
#   BASELINE=<csv folder> bash Petri/test/mcc/collect.sh 2026-09-07 LTLC LTLF
#
# Rsyncs each examination folder of ~/MCC26/MCC-drivers on the cluster into
# /data/ythierry/MCC26run/<date>/ (never with --delete: a partial campaign is collected as
# it drains, and rerunning adds the new logs), then runs the collectors over every
# examination folder present in <date>/: mcclogs2csv.py for the classic examinations,
...
# four minutes; the set whose glob names <date>/* in campaign/example.json picks the new
# logs up; a new campaign date needs a new set there first).
set -eu
HOST=${CLUSTER_HOST:-cluster.lip6.fr}
TREE=${CLUSTER_TREE:-MCC26/MCC-drivers}
RUNS=${RUNS:-/data/ythierry/MCC26run}
MCC=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
ANALYSIS=${ANALYSIS:-$HOME/git/MCC-analysis/campaign}
date=${1:?date, e.g. 2026-09-07}; shift
pages=0; exams=""
...
if [ $pages = 1 ]; then
  ...
  python3 "$ANALYSIS/build.py" "$ANALYSIS/example.json" > "$RUNS/pages-build-$date.log" 2>&1 && tail -2 "$RUNS/pages-build-$date.log"
  echo "pages: $RUNS/pages (python3 $ANALYSIS/serve.py $RUNS/pages --port 8080 to browse)"
fi
