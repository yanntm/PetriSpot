...
public :
	 ...
	 * @return
	 */
	static SparseArray sumProd(int alpha, const SparseArray & ta, int beta, const SparseArray & tb) {
		return sumProd(alpha, ta, beta, tb, -1);
	}
	static SparseArray sumProd(int alpha, const SparseArray & ta, int beta, const SparseArray & tb, int except) {
...
private :
	...
	template<typename U>
	    ...
	    template<typename U>
	    static std::pair<U*, size_t> insert(U* array, size_t aCap, size_t currentSize, size_t index, U element) {
	        assert(currentSize <= aCap);
	        if (currentSize + 1 <= aCap) {
