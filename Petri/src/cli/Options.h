...
struct Options
{
  ...
  uint64_t runTime = 1000;   // sweeps: wall clock budget of one run before a restart, ms (0: none)
...
inline void addOptions (CLI::App &app, Options &o)
{
  ...
  wk->add_option ("--seed", o.seed, "Random seed (default 1: two runs repeat up to thread timing; each walk call and each thread derive their own).");
