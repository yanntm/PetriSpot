...
template<typename T>
  ...
  public:
    Walker (const SparsePetriNet<T> &ssr)
        : combFlow (ssr.getPnames ().size (), 0)
    {
      this->sr = &ssr;
      typedef std::unordered_map<const SparseArray<T>*, std::vector<size_t>,
          std::hash<SparseArray<T>*>, std::equal_to<SparseArray<T>*>> map_t;
      map_t effects;

      for (size_t i = 0; i < sr->getFlowPT ().getColumnCount (); i++) {
        combFlow.appendColumn (
            SparseArray<T>::sumProd (-1, sr->getFlowPT ().getColumn (i), 1,
  ...
  public:
      ...
      for (int pi = 0, pie = static_cast<int>(tp.size ()); pi < pie; pi++) {
        ...
        if (tp.valueAt (pi) > 0) {
          ...
          for (size_t i = 0; i < col.size (); i++) {
            ...
            if (SparseArray<T>::greaterOrEqual (
                state, sr->getFlowPT ().getColumn (t))) {
              add (enabled, t);
              seen[t] = true;
              seenEffects[behaviorMap[t]] = true;
            }
    ...
    bool runDeadlockDetection (long nbSteps, bool fullRand, size_t timeout)
    {
      ...
      for (; i < nbSteps; i++) {
        size_t dur = std::chrono::duration_cast < std::chrono::milliseconds
            > (steady_clock::now () - time).count () + 1;
        if (dur > 1000 * timeout) {
          std::cout << "Interrupted Parikh directed walk after " << i
              << "  steps, including " << nbresets
              << " resets, run timeout after " << dur
        ...
        if (list[0] == 0) {
          ...
          if (list[0] == 0) {
            std::cout << "Finished Parikh directed walk after " << i
                << "  steps, including " << nbresets
                << " resets, run found a deadlock after " << dur
                << " ms. (steps per millisecond=" << (i / dur) << " )";
            if (DEBUG >= 1) {
              std::cout << " reached state ";
              state.print (std::cout);
        ...
        if (repeat) {
          ...
          do {
            ...
            i++;
          } while (SparseArray<T>::greaterOrEqual (
              state, sr->getFlowPT ().getColumn (tfired)));
        ...
        if (list[0] == 1 || fullRand || intRand (0, 100) >= 60) {
          ...
          for (int ti = 0; ti < list[0]; ti++) {
            ...
            if (listC[0] < minSucc
                || (listC[0] == minSucc && intRand (0, 100) >= 50)) {
              minSucc = listC[0];
              mini = ti;
              delete[] minList;
              minList = listC;
      ...
      size_t dur = std::chrono::duration_cast < std::chrono::milliseconds
          > (steady_clock::now () - time).count () + 1; // avoid zero divide
      std::cout << "Random " << (fullRand ? "" : "directed ") << "walk for "
          << i << "  steps, including " << nbresets << " resets, run took "
          << dur << " ms (no deadlock found). (steps per millisecond="
          << (i / dur) << " )";
      if (DEBUG >= 1) {
        std::cout << " reached state ";
        state.print (std::cout);
    ...
    {
      bool repeat = false;
      if (last != -1 && sr->getFlowPT ().getColumn (last).size () > 0
          && intRand (0, 100) < 98
          && SparseArray<T>::greaterOrEqual (
