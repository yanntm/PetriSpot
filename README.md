...
| `--threads=<n>`, `--tasks=<n>`, `--slice=<steps>`, `--sliceMs=<ms>` | runner threads, exploration tasks time-shared over them (default one per thread; more runs every strategy on every model), steps per slice and its wall-clock cap |
